...
public class Camera {
    ...
    /* ### MTK ADD-ONS: START ####
     * MediaTek proprietary Camera1 legacy API extensions.
     * Signatures reverse-engineered from com.mediatek.camera (vendor Camera.apk) smali,
     * required so the vendor camera app resolves its classpath and does not crash with
     * ClassNotFoundException / NoClassDefFoundError at load time.
     */
    ...
    private ZSDPreviewDone mZSDPreviewDone;

    /**
     * MTK callback interface for Auto Scene Detection (ASD).
     * @hide
     */
    public interface AsdCallback
    {
        void onDetected(int xy);
    };

    /**
     * Registers a callback to be invoked when the MTK ASD engine detects a scene.
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * MTK callback interface for auto-panorama (AutoRama) capture completion.
     * @hide
     */
    ...
    /**
     * Registers a callback to be invoked when an AutoRama (panorama) capture completes.
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * MTK callback interface for auto-panorama (AutoRama) frame/direction updates.
     * @hide
     */
    public interface AutoRamaMoveCallback
    {
        void onFrame(int xy, int direction);
    };

    /**
     * Registers a callback to be invoked on AutoRama (panorama) move/frame events.
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * Starts MTK AutoRama (panorama) capture in the given direction.
     * @param direction implementation-defined direction constant
     * @hide
     */
    public final void startAutoRama(int direction)
    {
    }

    /**
     * Stops MTK AutoRama (panorama) capture.
     * @param direction implementation-defined direction constant
     * @hide
     */
    public final void stopAutoRama(int direction)
    {
    }

    /**
     * MTK callback interface for continuous shot (burst) completion.
     * @hide
     */
    public interface ContinuousShotCallback
    {
        void onConinuousShotDone(int capNum);
    };

    /**
     * Registers a callback to be invoked when a continuous shot (burst) sequence completes.
     * @param cb The callback to run
     * @hide
     */
    public final void setContinuousShotCallback(ContinuousShotCallback cb)
    {
        mContinuousShotCallback = cb;
    }

    /**
     * Sets the MTK continuous shot (burst) capture speed.
     * @param speed implementation-defined speed constant
     * @hide
     */
    public final void setContinuousShotSpeed(int speed)
    {
    }

    /**
     * Cancels an in-progress MTK continuous shot (burst) capture.
     * @hide
     */
    public final void cancelContinuousShot()
    {
    }

    /**
     * MTK callback interface for stereo (dual camera) distance information.
     * @hide
     */
    ...
    /**
     * Registers a callback to be invoked with MTK stereo camera distance information.
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * MTK callback interface for Face Beauty original (unprocessed) image capture.
     * @hide
     */
    ...
    /**
     * Registers a callback to be invoked with the original (pre-Face-Beauty) JPEG image.
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * MTK callback interface for stereo (dual camera) capture data.
     * @hide
     */
    public interface StereoCameraDataCallback
    {
        void onJpsCapture(byte[] data);
        void onDepthMapCapture(byte[] data);
        void onClearImageCapture(byte[] data);
        void onLdcCapture(byte[] data);
        void onMaskCapture(byte[] data);
    };

    /**
     * Registers a callback to be invoked with MTK stereo camera capture data
     * (JPS, depth map, clear image, LDC, and mask buffers).
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * MTK callback interface for stereo (dual camera) warning notifications.
     * @hide
     */
    ...
    /**
     * Registers a callback to be invoked on MTK stereo camera warning conditions.
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * MTK callback interface for continuous autofocus (AF) data.
     * @hide
     */
    ...
    /**
     * Registers a callback to be invoked with MTK continuous autofocus data.
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * MTK callback interface for HDR original (unprocessed) image capture.
     * NOTE: no caller of this interface's method was found anywhere in the
     * vendor Camera.apk (type is only ever passed through, never implemented
     * or invoked), so this single-method shape is inferred from the naming
     * convention of the other MTK callbacks (in particular FbOriginalCallback)
     * rather than confirmed from decompiled bytecode.
     * @hide
     */
    ...
    /**
     * Registers a callback to be invoked with the original (pre-HDR) JPEG image.
     * @param cb The callback to run
     * @hide
     */
    ...
    /**
     * MTK callback interface for Zero Shutter Delay (ZSD) preview-done notification.
     * NOTE: no caller of this interface's method was found anywhere in the
     * vendor Camera.apk (type is only ever passed through, never implemented
     * or invoked), so this single-method, no-argument shape is inferred rather
     * than confirmed from decompiled bytecode.
     * @hide
     */
    ...
    /**
     * Registers a callback to be invoked when MTK ZSD preview processing is done.
     * @param cb The callback to run
     * @hide
     */
    public final void setPreviewDoneCallback(ZSDPreviewDone cb)
    {
        mZSDPreviewDone = cb;
    }
    /* ### MTK ADD-ONS: END ### */
